...
public class NewWorldSpawn : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject[] Grounds = GameObject.FindGameObjectsWithTag("Земля");
        for(int i = 0; i < Grounds.Length; i++)
        {
            Grounds[i].name = ("Земля " + i);
        }

        GameObject[] Sands = GameObject.FindGameObjectsWithTag("Пустыня");
        for(int i = 0; i < Sands.Length; i++)
        {
            Sands[i].name = ("Пустыня " + i);
        }
    }
}
